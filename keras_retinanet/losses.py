...
def focal_loss(alpha=0.25, gamma=2.0):
    def _focal_loss(y_true, y_pred):
        ...
        batch_size = 6
        #batch_size = keras.backend.int_shape(y_pred)[0]
        cls_loss_total = 0
        if not batch_size:
            batch_size = 1
...
def regression_loss(y_true, y_pred):
    ...
    if not batch_size:
        batch_size = 1
    print("BSIZE: {}".format(batch_size))
    for b in range(0,batch_size):
        regression        = y_pred[b, :, :]
        regression_target = y_true[b, :, :4]
        labels            = y_true[b, :, 4:]
        ...
        regression_loss_total += regression_diff / divisor

    return regression_loss_total
